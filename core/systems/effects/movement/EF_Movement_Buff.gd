...
## 2. Set the desired speed multiplier (e.g. 1.5 for 150% speed)
## 3. Set the duration of the effect (default is 3 seconds)
## 4. Pass Effect to scheduler
##
## How it works:
## On enter, this effect stores the player's current move_speed and applies a multiplier buff.
...
## Init
func _init():
	self.set_effect_name("Movement - Buff")
...
## Applies movement buff to the player.
func _apply_effect() -> void:
	...
	if is_debug_enabled():
		_log_generic("_apply_effect - applying movement buff. move_speed: " + str(_original_move_speed) + " -> " + str(new_speed))
	player.move_speed = new_speed
	_is_applied = true

## Reverts movement buff on the player.
...
## override enter() to apply movement buff when this effect starts
